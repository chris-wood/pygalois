...
class GFElem:
	...
	def isUnit(self):
		if (len(self.coeff) == 0):
			return False
		elif (self.coeff[0] == 1 and len(self.coeff) == 1):
			return True
		else:
			return False
...
class GFExtension:
	...
	def findGenerators(self, earlyTerm = False):
		...
		gens = []
		for i in range(2**self.getN(), fieldOrder): # start with at least one y coefficient
			try:
				elemOrder = self.findOrder(i)
